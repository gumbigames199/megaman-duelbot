...
export async function execute(ix: ChatInputCommandInteraction) {
  const wantRegion = ix.options.getString('region_id') ?? undefined;
  const wantZone   = ix.options.getInteger('zone') ?? undefined;

  const bundle = getBundle();

  // If region specified, switch region (must be unlocked) and reset zone to 1
  if (wantRegion) {
